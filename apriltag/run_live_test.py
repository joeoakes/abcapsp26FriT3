...
def dashboard():
    ...
    <head>
        ...
        <style>
            ...
            /* Added for TC-071 Usability Test */
            #status-banner {
                background: #444; padding: 15px; margin: 20px auto; width: 80%;
                border-radius: 8px; font-size: 24px; font-weight: bold; transition: background 0.3s;
            }
            .detected { background: #2e7d32 !important; } /* Green when detected */
            .searching { background: #c62828 !important; } /* Red when lost */
        </style>
            ...
            function fetchStatus() {
                fetch('/detection')
                    ...
                    .then(data => {
                        ...
                        if (data.tag_id !== null) {
                            banner.innerHTML = `✅ Tag ${data.tag_id} Detected | Distance (Z): ${data.z.toFixed(2)}m`;
                            banner.className = 'detected';
                        } else {
                            banner.innerHTML = '⚠️ No Tags in View';
                            banner.className = 'searching';
                        }
                    })
                    .catch(error => console.error('Error fetching status:', error));
            }
            // Update status every 500 milliseconds
    ...
    <body>
        ...
        <div id="status-banner" class="searching">Connecting to Vision System...</div>
